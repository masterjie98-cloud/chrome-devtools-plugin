...
export function buildAgentExecutionStrategyPrompt(): string {
  return [
    ...
    "For continuous current-Tab monitoring, start browser_activity_start once and retain activityCursor.streamId plus activityCursor.sequence. For each later user request asking what changed, call browser_debug_activity exactly once with afterStreamId and afterSequence set from that saved cursor, summarize every retained event in that returned window, and let the client commit activity.nextCursor only after the final summary succeeds. If cursorStatus=events_dropped, explicitly report missedEvents. If transportDroppedEvents contains non-zero counts, explicitly report the local daemon transport gap. Never claim complete coverage after either condition. Never poll browser_debug_activity again in the same response, including when the result is empty, and never omit afterSequence as a supposed full-history recovery.",
