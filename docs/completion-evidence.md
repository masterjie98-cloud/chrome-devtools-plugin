...
| V5 execution core: task grants, compact incremental DOM, action stage, adaptive visual, Network observation and lifecycle timing | proven | `docs/execution-core-optimization-plan.md`; dynamic policy and exact-bound grant tests; V5 schema/registry checks; action-stage batching test; MutationObserver delta and visual checkpoint tests; actual queue/transport/approval/executor/total audit fields; 296/296 suite; packaged 69-tool process verification; and the 2026-07-17 real-Chrome evidence below | None for the declared execution-core scope. The broader Architecture V2 Profile/OOPIF/artifact worksheet remains separate. |
...
| --- | --- | --- | --- |
| Static types | proven | Final `npm run verify:packaged` reran TypeScript `--noEmit` successfully on 2026-07-17 | Rerun after any later code change. |
| Unit/integration | proven | The final complete `tsx --test tests/*.test.ts` suite passed 296/296 on 2026-07-17, including task capability binding/expiry/revoke, dynamic approval modes, bounded action-stage exposure and dependency-aware batching, same-target approval enrichment, grant lifecycle audit schemas, actual queue timing, large-DOM visual checkpoints, delegated-task durability, trusted input, Network digest, routing, state and recovery coverage. | Rerun after any later code change. |
| Production build | proven | Final TypeScript, Vite, content script, daemon, MCP adapter, and status command builds passed on 2026-07-17 | Rerun after any later code change. |
...
| Approval | partial | safe-read policy plus sensitive/mutation approve/deny/grant tests; conversation-origin chat/origin/Profile/sidepanel/Provider boundaries and dangerous-class exclusions; real fixed-card visibility, enable, cross-tool same-origin reuse, manual revoke, and origin expiry/re-prompt | Real active-grant chat-switch expiry and unavailable-UI behavior. |
...
| Data egress | partial | cookie/storage defaults; header/body/query/fragment redaction; class/UTF-8 byte metrics; artifact byte metrics; fast mode defaults off, is disabled by Safe Retry, never auto-attaches a page screenshot on message send, requires an approval-gated Agent screenshot request to activate automatic visual follow-ups, and then permits at most 8 latest-only sampled-fingerprint-deduplicated adaptive attempts within an 8 MiB image budget | Real initial request with zero screenshots, Agent-requested screenshot approval, adaptive route/drawer/large-DOM checkpoint delivery after activation, Provider-origin-change confirmation, and the remaining sensitive-read destination cases in the Side Panel. |
