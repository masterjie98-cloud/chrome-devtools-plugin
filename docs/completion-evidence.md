...
| --- | --- | --- | --- |
| Static types | proven | `npm run build` reran TypeScript `--noEmit` successfully on 2026-07-28 | Rerun after any later code change. |
| Unit/integration | proven | The complete `tsx --test tests/*.test.ts` suite passed 370/370 on 2026-07-28, including result arbitration, unified recovery, safe-read reconnect, unknown-write non-replay, conditional workflow, collaboration V2, issue-evidence export, source/performance diagnostics and conversation search/export. | Rerun after any later code change. |
| Production build | proven | TypeScript, Vite, content script, daemon, MCP adapter, and status command builds passed on 2026-07-28 | Rerun after any later code change. |
| Daemon lifecycle | proven | 2026-07-28 `npm run verify:packaged`: actual dist daemon, authenticated status, clean shutdown, private config, same-config restart; the user LaunchAgent was then reinstalled and `daemon:status` returned `ok: true` | Rerun the verifier after lifecycle or packaging changes. |
| MCP multi-instance | proven | 2026-07-28 `npm run verify:packaged`: 86 exposed tools, two simultaneous actual `dist/mcp/server.js` processes with distinct PIDs; one exits independently and the daemon survives adapter exit | Rerun the verifier after adapter transport or lifecycle changes. |
