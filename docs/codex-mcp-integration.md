...
- 未接受任务可跨对话在收件箱中处理。接受后，`taskId` 绑定到当时的插件对话和
  固定 Tab；切换 Tab、切换插件对话或新建干净对话都不会改变这组绑定。
- 新建或切换对话不会显示其他对话已接受、完成、失败或取消的 Codex 卡片；回到
  原对话仍可看到该卡片、进度和结果。原任务可以继续在后台运行，新对话的消息
  会形成独立任务并保留自己的对话/Tab 绑定。
- 每个 MCP 线程或独立工作必须使用不同的 `taskId`。后续 progress、requirement、
  evidence、wait 和 result 都按 `taskId` 路由，不按用户当前正在查看的对话路由。
...
如果用户删除了已绑定的旧对话，daemon 不会删除 MCP 任务，也不会把后续 MCP
消息塞进当前打开的任意对话。该 `taskId` 会作为“待恢复任务”重新进入 Profile
级收件箱：

1. progress、requirement、evidence 和最终结果继续保存在原 `taskId` 下；
2. 用户点击“恢复到新对话”后，插件创建干净对话，并使用
   `resume=true, rebind=true` 把原任务显式迁移过去；
3. 新对话继续绑定原任务目标 Tab，claim 记录旧 conversation key，并强制
   `requiresReobservation=true`；
4. 用户没有显式恢复前，任务不会自动执行，也不会污染当前聊天。

升级前已经 claim、但没有 `conversationId` 绑定的旧任务同样进入全局收件箱。
未绑定终态任务不会投影到任意新对话，但 Codex 仍可用原 `taskId` 读取持久化结果。
...
32 KiB，整个 workspace 的本地持久化/重放硬上限为 256 KiB。这个值不是模型
上下文窗口，也不是 MCP 协议上限。

正常写入会在约 208 KiB 开始主动压缩，为一条最大内联内容和元数据预留空间。
压缩按父任务分组，优先保留本次写入、尚未完成的委托和最近的委托结果；旧终态
记录优先淘汰，失去父项的孤儿记录不会继续持久化。只有单条 content 超过 32 KiB，
或一个不可拆分任务组自身超过 256 KiB 时才返回 `PAYLOAD_TOO_LARGE`。完整 DOM、
截图二进制、凭据和敏感 Network body 不应直接塞进协作项，应保存为受 Profile
约束的 artifact，并在协作项中只发布 URI 和摘要。
